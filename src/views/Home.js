  ...
  return (
    <div className="p-6">
      {alert.visible && alert.type === 'apiKeyError' && (
        <ErrorAlert message="API Key is not set, please go to settings." />
      )}
      ...
      />
      {inputError.value && inputError.inputName === 'prompt' && (
        <p className="mt-1 text-xs text-red-600">The prompt cannot be empty!</p>
      )}

      <label htmlFor="exportFormat" className="block mt-5 text-sm font-bold text-gray-900">
        Choose an export format
      </label>
      <ul
        id="exportFormat"
        className="items-center mt-2 w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg">
        {['csv', 'json'].map((type) => (
          <li key={type} className="w-full border-b border-gray-200">
            <div className="flex items-center ps-3">
              ...
              />
              <label
                htmlFor={`horizontal-list-radio-${type}`}
